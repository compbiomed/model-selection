...
predicttion.single.best <- predictAndEvaluation(model.best = model.selection.output$list.model.fit$gbm,
                                    test.data = data.norm$testTransformed,
                                    prevalence = 0.1,
                                    is.ensemble = FALSE)

# The performance of the single best model, given sensitivity and specificity thresholds
predicttion.single.best.2 <- predictAndEvaluationSen(model.best = model.selection.output$list.model.fit$gbm,
                                    test.data = data.norm$testTransformed,
                                    prevalence = 0.1,
                                    is.ensemble = FALSE,
                                    sen_min = 0.7,
